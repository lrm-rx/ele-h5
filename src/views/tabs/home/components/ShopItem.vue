...
function gotoShop(id: number | string) {
  router.push({
    name: 'shop',
    params: {
      id,
...
<template>
  <div class="home-shop-item" @click="gotoShop(data.id)">
    <img v-lazy="data.postUrl" class="home-shop-item__poster">
    <div class="home-shop-item__info">
      <div class="info__top">
        <div class="info__name op-ellipsis">
          {{ shopName }}
        </div>
        <VanIcon name="weapp-nav" color="rgb(207, 207, 207)" style="margin-left: 10px" />
      </div>
      ...
      <div class="info__more" @click.stop="showMore">
        <div v-if="isMoreShow" class="label">
          活动
        </div>
        <div class="activities">
          <div v-if="!isMoreShow" class="activity op-thin-border">
            {{ reduction[0] }} | {{ reduction[1] }}
          </div>
          <template v-else>
            <div v-for="v in reduction" :key="v" class="activity op-thin-border">
              {{ v }}
            </div>
          </template>
        </div>
        <VanIcon :name="`arrow-${isMoreShow ? 'up' : 'down'}`" color="rgb(207, 207, 207)" />
      </div>
    </div>
  </div>
</template>
